...
export default function MerchantManagement() {
  ...
  const { data: merchants } = useQuery({ queryKey: ["admin-merchants"], queryFn: admin.merchants.list });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Input className="max-w-xs h-8 text-sm" placeholder="Search merchants…" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
      <Card>
            ...
            <tbody>
              {filtered?.length ? filtered.map((m) => (
                <tr key={m.id} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-2"><Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => toggleMut.mutate({ id: m.id, enabled: m.status !== "active" })}>{m.status === "active" ? "Disable" : "Enable"}</Button></td>
                </tr>
