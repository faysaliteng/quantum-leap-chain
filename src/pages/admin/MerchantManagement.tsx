    ...
    <div className="space-y-4" data-testid="page:admin-merchants">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Input className="max-w-xs h-8 text-sm" placeholder={t("admin.searchMerchants")} value={search} onChange={(e) => setSearch(e.target.value)} maxLength={100} />
        </div>
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard label={t("admin.totalMerchants")} value={merchants?.length ?? 0} icon={Users} />
        <StatCard label={t("admin.active")} value={active} icon={UserCheck} />
        <StatCard label={t("admin.disabled")} value={disabled} icon={UserX} />
      </div>
      <Card>
        <CardContent className="p-0">
          {isLoading ? <TableSkeleton /> : (
            <table className="w-full text-sm">
              <thead><tr className="border-b text-left text-xs text-muted-foreground uppercase"><th className="px-4 py-2">{t("table.name")}</th><th className="px-4 py-2">{t("auth.email")}</th><th className="px-4 py-2">{t("table.status")}</th><th className="px-4 py-2">{t("table.created")}</th><th className="px-4 py-2"></th></tr></thead>
              <tbody>
                {filtered?.length ? filtered.map((m) => (
                  <tr key={m.id} className="border-b last:border-0">
                    ...
                    <td className="px-4 py-2"><Button variant="outline" size="sm" className="h-7 text-xs" disabled={toggleMut.isPending} onClick={() => toggleMut.mutate({ id: m.id, enabled: m.status !== "active" })}>{m.status === "active" ? t("admin.disable") : t("admin.enable")}</Button></td>
                  </tr>
                )) : <tr><td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">{t("admin.noMerchants")}</td></tr>}
              </tbody>
            </table>
          )}
        </CardContent>
