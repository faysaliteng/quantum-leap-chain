...
export default function SystemMonitoring() {
  ...
  if (!health) return <p className="text-muted-foreground">{t("admin.unableToFetch")}</p>;
    ...
    <div className="space-y-6" data-testid="page:admin-monitoring">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => startExport("health_snapshot", "json")} disabled={isExporting}>
          <FileDown className="mr-1.5 h-3.5 w-3.5" />{isExporting ? t("charges.exporting") : t("admin.snapshot")}
        </Button>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-sm">{t("admin.watcherCheckpoints")}</CardTitle></CardHeader>
        <CardContent className="p-0">
          <table className="w-full text-sm">
            <thead><tr className="border-b text-left text-xs text-muted-foreground uppercase"><th className="px-4 py-2">{t("table.chain")}</th><th className="px-4 py-2">{t("admin.block")}</th><th className="px-4 py-2">{t("admin.latest")}</th><th className="px-4 py-2">{t("admin.lag")}</th><th className="px-4 py-2">{t("admin.updated")}</th></tr></thead>
            <tbody>{health.watchers.map((w) => (
              <tr key={w.chain} className="border-b last:border-0">
                ...
                <td className="px-4 py-2"><Badge variant={w.lag > 10 ? "destructive" : w.lag > 3 ? "outline" : "default"} className="text-xs font-mono">{w.lag} {t("admin.blocks")}</Badge></td>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardHeader><CardTitle className="text-sm">{t("admin.rpcHealth")}</CardTitle></CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between text-sm"><span>{t("admin.pending")}</span><span className="font-mono">{health.webhook_queue.pending}</span></div>
            <div className="flex justify-between text-sm"><span>{t("admin.failed")}</span><span className="font-mono text-destructive">{health.webhook_queue.failed}</span></div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-sm">{t("admin.uptime")}</CardTitle></CardHeader>
          <CardContent>
